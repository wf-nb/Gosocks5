...
function Update_Shell() {
    ...
    Shell_NewVer=$(wget --no-check-certificate -qO- "https://github.com/wf-nb/Gosocks5/raw/main/gosocks5.sh"|grep 'Shell_Version="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
    ...
    if [ $(awk -v Shell_NewVer="$Shell_NewVer" -v Shell_Version="$Shell_Version"  'BEGIN{print(Shell_NewVer>Shell_Version)?"1":"0"}') ]; then
        ...
        [[ -z "${Read_YN}" ]] && Read_YN="Y"
        if [[ ${Read_YN} == [Yy] ]]; then
            wget -N --no-check-certificate https://github.com/wf-nb/Gosocks5/raw/main/gosocks5.sh && chmod +x gosocks5.sh
